package com.study.camel.simplecamelspringboot.components;

import com.study.camel.simplecamelspringboot.beans.Client;
import org.apache.camel.LoggingLevel;
import org.apache.camel.builder.RouteBuilder;
import org.apache.camel.dataformat.beanio.BeanIODataFormat;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.stereotype.Component;

@Component
public class LegacyFileCSVSplitterBeanRoute extends RouteBuilder {

    Logger logger = LoggerFactory.getLogger(getClass());
    BeanIODataFormat inboundDataFormat = new BeanIODataFormat("inboundMessageBeanIoMapping.xml","inboundMessageStream");

    @Override
    public void configure() throws Exception {
        from("file:src/datad/input?fileName=inputFile.csv")
                .id("legacyFileTransferWithSplitterBeanRoute")
                .split(body().tokenize("\n", 1, true))
                .unmarshal(inboundDataFormat)
                .process(exchange -> {
                    Client fileData = exchange.getIn().getBody(Client.class);
                    logger.info(fileData.toString());
                    //removing some characters
//                    fileData = fileData.replaceAll("i","1");
//                    exchange.getIn().setBody(fileData.toString()); //we can use convertBodyTo
                })
                .convertBodyTo(String.class)
                .log(LoggingLevel.INFO, "moving the file")
                .to("file:src/datad/output?fileName=output.csv&fileExist=append&appendChars=\\n")
                .end(); //to indicate what block is executed in split
    }
}
